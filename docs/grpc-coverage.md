...
| OCaml | ⏳ Codegen rule exists but excluded from the build | `modules/generated/dune` (excluded via `backend-ocaml/dune`'s `(subdir modules (dirs :standard \ generated))`) | The rule's `(deps ../../proto/ecommerce.proto)` reaches outside both the Docker build context (scoped to `backend-ocaml/` only) and dune's own project-root sandboxing — it can't work as written inside this repo's layout. Excluded rather than left to fail every build; `grpc`/`grpc-lwt` remain the highest-risk remaining target if this is revisited. |
